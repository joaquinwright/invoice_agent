...
The schema is deterministic: a given input either parses into a valid Receipt or
raises a clear error, which makes it a good fit for classic unit tests (unlike the
LLM step, which the eval measures instead).
...
# A reusable, fully-valid receipt as a plain dict. line_items + total are required;
# quantity, subtotal, tax are optional.
VALID = {
    "total": 8.50,
    "subtotal": 8.50,
    "tax": 0.0,
...
def test_numeric_string_is_coerced_to_float():
    # Pydantic coerces a clean numeric string like "8.50" to a float. That's still
    # only validation (well-formedness), not a claim the value is correct.
...
def test_malformed_json_string_raises():
    # model_validate_json takes a raw string (what an LLM returns). A chatty preamble
    # before the JSON makes it unparseable.
